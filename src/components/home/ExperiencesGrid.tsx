...
export default function ExperiencesGrid() {
  ...
  const copy = (slug: string) => dict.experiences[slug as keyof typeof dict.experiences] ?? { title: slug, description: '' };
  ...
  return (
    <section className="bg-brand-cream py-28 lg:py-36">
      <div className="mx-auto max-w-7xl px-6 lg:px-10">
        <SectionHeading
          i18n={{
            eyebrow: 'home.gridKicker',
            title: 'home.gridTitle',
            description: 'home.gridDescription',
          }}
        />
            ...
            <Link href="/experiencias" className="group relative block overflow-hidden">
              <div className="relative aspect-[21/9] min-h-[280px]">
                ...
                <div className="absolute bottom-0 left-0 p-8 md:p-12 max-w-xl">
                  <p className="text-[11px] tracking-[0.4em] uppercase text-brand-gold font-body mb-3">{t('home.featured')}</p>
                  <h3 className="font-display text-4xl md:text-5xl text-brand-cream mb-3">{copy(featured.slug).title}</h3>
                  <p className="font-body text-brand-cream/75 leading-relaxed">{copy(featured.slug).description}</p>
                </div>
              </div>
            </Link>
          </ScrollReveal>
        ...
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {rest.map((exp, i) => (
            <ScrollReveal key={exp.slug} delay={i * 0.06}>
              <Link href="/experiencias" className="group relative block overflow-hidden bg-brand-black">
                <div className="relative aspect-[3/4]">
                  <Image
                    src={exp.src}
                    alt={copy(exp.slug).title}
                    fill
                  ...
                  <div className="absolute bottom-0 inset-x-0 p-6 md:p-7">
                    <p className="text-[10px] tracking-[0.35em] uppercase text-brand-gold/90 font-body mb-2">0{i + 2}</p>
                    <h3 className="font-subtitle text-2xl text-brand-cream">{copy(exp.slug).title}</h3>
