...
function buildApplicationMessage(data: CareersFormData, puestoLabel: string, cvName?: string): string {
  return [
    `Nueva postulación laboral — Aguas del Cerro`,
    '',
    `Nombre: ${data.nombre}`,
...
async function submitViaFormSubmitAjax(
  ...
  try {
    ...
    return { ok: false, error: 'No se pudo enviar la postulación. Intentá de nuevo; si se repite, escribinos por WhatsApp.' };
  } finally {
...
function CareersFormFields({
  ...
  const onSubmit = async (data: CareersFormData) => {
    ...
    try {
      ...
      const apiTimer = window.setTimeout(() => apiController.abort(), 12_000);
      let apiError: string | undefined;

      try {
        const response = await fetch('/api/careers', { method: 'POST', body, signal: apiController.signal });
        const result = (await response.json().catch(() => null)) as {
          ok?: boolean;
          error?: string;
          message?: string;
      ...
      setSubmitError(
        apiError ??
          browserSend.error ??
          'No se pudo enviar la postulación. Intentá de nuevo; si se repite, escribinos por WhatsApp.',
      );
    } catch {
